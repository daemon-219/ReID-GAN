...
__all__ = ['ResNetBIPD', 'resnet_bipd50']


class ResNetBIPD(nn.Module):
    __factory = {
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
        ...
        super(ResNetBIPD, self).__init__()
        self.pretrained = pretrained
        self.depth = depth
        self.cut_at_pooling = cut_at_pooling
        self.need_predictor = need_predictor
        # Construct base (pretrained) resnet
        if depth not in ResNetBIPD.__factory:
            raise KeyError("Unsupported depth:", depth)
        if self.pretrained:
            resnet = ResNetBIPD.__factory[depth](weights=torchvision.models.ResNet50_Weights.IMAGENET1K_V1)
        else:
            resnet = ResNetBIPD.__factory[depth]()
        ...
        self.p1 = nn.Sequential(copy.deepcopy(resnet.layer4))
        ...
        if not self.cut_at_pooling:
            ...
            if self.has_embedding:
                ...
                self.feat_bn2 = nn.BatchNorm2d(self.num_features)
    ...
    def forward(self, x, fuse=True, lambda_fuse=0.8):
        bs = x.size(0)
        x = self.base(x)
    ...
    #     bn_x = lambda_fuse * bn_x1 + (1-lambda_fuse) * self.gap(bn_x2).view(bs, -1)
...
def resnet_bipd50(**kwargs):
    return ResNetBIPD(50, **kwargs)
