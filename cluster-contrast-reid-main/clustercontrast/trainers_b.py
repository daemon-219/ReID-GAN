...
class GANTrainer(object):
    ...
    def train_gan_with_reid_enc(self, epoch, data_loader, print_freq=10, train_iters=400, acc_iters=0):
        ...
        for i in range(train_iters):
            ...
            if self.opt.bipath:
                _, f_reid = self.encoder(my_transform(gan_inputs['Xs']), fuse=False)
            
            # print('f_reid.shape', f_reid.shape)
...
class ClusterContrastWithGANTrainer(object):
    ...
    def train_all(self, epoch, data_loader, optimizer, print_freq=10, train_iters=400, acc_iters=0, conf_weight=None):
        ...
        for i in range(train_iters):
            ...
            # cluster_images = self.gan.synthesize_id(f_out, self.memory.features, labels)
            """
            TODO: gan forward
            ...
            # lambda_fus = self.opt.lambda_fus + (epoch / self.opt.epochs) * 0.3
            ...
            # hardest in cluster samples mixup with nearest cluster
            # uids = torch.unique(labels)
            # sim = torch.exp(torch.einsum('n c, m c -> n m', [self.memory.features[uids], self.memory.features]))
            # sim[torch.arange(uids.shape[0]), uids] += -10000.0
            # # select the nearest out id
            # out_id = torch.argmax(sim, dim=1)
            # in_id = torch.argmin(conf_mask.reshape(uids.shape[0], group_size), dim=1)
            # f_ex = F.normalize(self.opt.lambda_fus * f_out[in_id] + (1- self.opt.lambda_fus) * self.memory.features[out_id])

            if conf_weight is not None:
                conf_mask = conf_weight[indexes]
            else:
                conf_mask = torch.ones_like(indexes).cuda()

            # conf_mask = torch.ones_like(indexes).cuda()
            # # 获取出现次数最多的元素
            # real_labels = inputs[1]['gt_label'].cuda().reshape(-1, group_size)
            # most_frequent_id, _ = torch.mode(real_labels, keepdim=True)
            # # print(real_labels)
            # # print(most_frequent_id)
            # conf_mask *= (real_labels == most_frequent_id).reshape(-1)
            # # print(conf_mask.reshape(-1, group_size))

            # # blocked_num = int((group_size // 2) * (1 - epoch / self.opt.epochs))
            # blocked_num = torch.div(group_size, min(torch.pow(torch.tensor(2), 1 + epoch), group_size), rounding_mode='trunc')
            # _, blocked_ids = torch.topk(cluster_l1.reshape(-1, group_size), blocked_num)
            # # print(blocked_ids)
            # conf_mask = conf_mask.reshape(-1, group_size).scatter(-1, blocked_ids, 0.)
            # # print(group_size.dtype)
            # # print(blocked_num.dtype)
            # # conf_mask *= group_size / (group_size - blocked_num)

            loss_cl = self.memory(f_out, labels)
            # loss_cl = self.memory(f_out, labels, conf_mask=conf_mask.reshape(-1))

            if i == 0:
                b_1 = inputs[1]['gt_label'][:group_size].cuda()
                X = torch.sort(loss_cl[:group_size], -1, descending=True)
                print("cl loss", loss_cl[:group_size])
                print("conf mask", conf_mask[:group_size].gather(-1, X[1]))
                print("gt_label_cl", b_1.gather(-1, X[1]))

                Y = torch.sort(conf_mask[:group_size], -1, descending=True)
                # Y = torch.sort(-cluster_l1[:group_size], -1, descending=True)
                print("gt_label_rec", b_1.gather(-1, Y[1]))

            loss_cl *= conf_mask
            # loss_cl *= conf_mask.reshape(-1)

            loss_cl = loss_cl.mean()
            # loss_cl = self.memory(f_out, labels, ex_f=f_ex.detach(), conf_mask=conf_mask)
            # loss_cl = self.memory(f_out, labels, ex_f=f_ex.detach(), conf_mask=conf_mask, focus_hard=(epoch>25))
            
            # loss = loss_cl
            loss = loss_cl + loss_G
            # loss = self.opt.lambda_nl * loss_cl + (1 - self.opt.lambda_nl) * loss_G
    ...
    def train_all_bip(self, epoch, data_loader, optimizer, print_freq=10, train_iters=400, acc_iters=0, conf_weight=None):
        print("train bipath")
        # self.encoder.eval()
        self.encoder.train()
        ...
        for i in range(train_iters):
            ...
            f_out1, f_out2 = self._forward(reid_inputs, fuse=False)
            ...
            """
            # fake_images = self.gan.synthesize_p(f_out2)

            mixed_images = self.gan.synthesize_mhp_bi(f_out2, f_out1.detach(), self.memory.features, labels, self.opt.lambda_fus, group_size)

            self.encoder.eval()
            f_ex = self._forward(my_transform(mixed_images), fuse=True)
            self.encoder.train()
            ...
            # f_out2_gp = self.encoder.module.gap(f_out2).view(f_out2.shape[0], -1)

            # loss_intra = self.intra_cl(f_out2_gp, f_out2_gp.detach(), group_size=group_size)

            # _, fc, fh, fw = f_out2.shape
            # temp_cluster = torch.mean(f_out2.reshape(-1, group_size, fc, fh, fw), dim=1)
            ...
            # loss_cl = self.memory(f_out2, labels, conf_mask=conf_mask)
            # loss_cl1 = self.memory(f_out1, labels)
            loss_cl1 = self.memory(f_out1, labels, ex_f=f_ex.detach())
            # loss_cl2 = self.memoryb(f_out2, labels).mean()
            ...
            # _, blocked_ids = torch.topk(cluster_l1.reshape(-1, group_size), blocked_num[(epoch // 20)])
            # # print(blocked_ids)
            ...
            # if i == 0:
                # b_1 = inputs[1]['gt_label'][:group_size].cuda()
                # X = torch.sort(loss_cl1[:group_size], -1, descending=False)
                # print("cl loss", X[0])
                # # print("conf mask", conf_mask[:group_size].gather(-1, X[1]))
                # print("gt_label_cl", b_1.gather(-1, X[1]))
            ...
            # loss_dist = self.opt.lambda_nl * self.f_metric(self.encoder.module.gap(f_out2.detach()).view(f_out2.shape[0], -1), self.encoder.module.predictor(f_out1))

            loss_cl = loss_cl1.mean()
            # loss_cl = self.memory(f_out, labels, ex_f=f_ex.detach(), conf_mask=conf_mask)
            # loss_cl = self.memory(f_out, labels, ex_f=f_ex.detach(), conf_mask=conf_mask, focus_hard=(epoch>25))
            
            # loss = loss_cl1 + loss_cl2
            loss = loss_cl + loss_G
            # loss = loss_cl1
            # loss = self.opt.lambda_nl * loss_cl + (1 - self.opt.lambda_nl) * loss_G
    ...
    def intra_cl(self, q, k, group_size=16):
        ...
        logits = torch.einsum('n c, m c -> n m', [q, k]) / self.T
        qs, ks = logits.shape
        logits = torch.sum(logits.reshape(-1, group_size, ks), dim=1)
        N = logits.shape[0]  # batch size per GPU
        labels = torch.arange(N, dtype=torch.long).cuda()
